...
class Flink(PndaPlugin):
    ...
    def read_args(self, args):
        ...
        parser = argparse.ArgumentParser(prog=self.__class__.__name__,
                                         usage='%(prog)s [options]', description='Show state of Flink History server')
    ...
    @staticmethod
    def validate_api_response(response, path, other_exp_codes=None):
        expected_codes = [200]
    ...
    def runner(self, args, display=True):
        ...
        values.append(Event(TIMESTAMP_MILLIS(), "flink",
                            "flink.history_server_available_success", [], hs_available_success))
        ...
        values.append(Event(TIMESTAMP_MILLIS(), "flink",
                            "flink.history_server_completed_jobs_success", [], hs_completed_jobs_success))

        values.append(Event(TIMESTAMP_MILLIS(), "flink",
                            "flink.history_server_completed_jobs_count", [], completed_job_count))

        values.append(Event(TIMESTAMP_MILLIS(), "flink",
                            "flink.history_server_completed_jobs_ms", [], hs_completed_jobs_ms))
